        ...
        if(s3res.statusCode === 200) {
          ...
          var handleValidation = function(error, hasNew) {
            ...
            if (countMatches && !hasNew) {
              ...
              // res.set(s3res.headers);
              // s3client.get(name).on('response', function(s3data) {
              //   s3data.on('data', function(chunk){
              //     console.log(chunk);
              //   });
              // }).end();

              console.log("REDIRECT", s3res.req.url)
              res.redirect(307, s3res.req.url);
            }else {
          ...
          // We need to render a tile.
          console.log("CACHE MISS", s3res.statusCode, req.url);
          return handleRender(req, res, next);
        }

...
module.exports = function setup(options) {
  ...
  function getTimestamp(survey, tile, done) {
    var query = setupQuery(survey, tile);
    Response.find(query)
    .sort({ created: -1 })
    .limit(1)
    .exec(function (error, docs) {
      ...
      if (docs.length === 0) {
        ...
        done(null, docs[0].created.getTime());
      }
    });
  }
  ...
  return function useCache(req, res, next) {
    ...
    async.parallel([
      // Check the timestamp of the latest item
      __.bind(getTimestamp, this, survey, tile),
      // Check if the key is in S3
      __.bind(getS3Header, this, name)
    ], function (error, values) {
      if (error) {
        console.log('Error validating or accessing cache.');
        console.log('MISS: unable to validate or unable to retrieve headers from S3');
        stopMetric.miss();
      ...
      // valid.
      cacheTimestamp = Date.parse(s3res.headers['last-modified']);
      if (cacheTimestamp <= dataTimestamp) {
        // There are new responses, so we need to render a tile.
        console.log('MISS: stale cached tile (new entries): ' + cacheTimestamp + ' <= ' + dataTimestamp, req.url);
        stopMetric.miss();
        return handleRender(req, res, next);
      }

      // There are no new entries. Check the count, in case there have been
      // deletions.
      getCount(survey, tile, function (error, trueCount) {
        if (error) {
        ...
        // Set custom count header, which we will later store along with the
        // response body on S3.

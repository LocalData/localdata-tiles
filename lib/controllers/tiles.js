...
function getOrCreateMapForSurveyId(surveyId, callback, options) {
  ...
  var select = {
    'geometry': 1,
    'properties.humanReadableName': 1,
    'properties.object_id': 1
  };

  // Add fields based on datasource
  if(options.key !== undefined) {
    select['properties.entries.responses.' + options.key] = 1;

    if(options.val !== undefined) {
      query['properties.entries.responses.' + options.key] = options.val;

      if (options.val === NOANSWER) {
        query['properties.entries.responses.' + options.key] = { "$exists": false };
      }
    }
  }
  ...
  if (options.key === 'Collectors') {
    // TODO: Get the list of all collectors and style the map appropriately

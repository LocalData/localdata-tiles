 ...
 */
formSchema.static('mostRecentForm', function (surveyId, cb) {
  return this.find({survey: surveyId}).sort({created: 'asc'}).limit(1).exec(cb);
});
 ...
 */
formSchema.static('getFlattenedForm', function(surveyId, cb) {
  this.mostRecentForm(surveyId, function(error, doc){
    ...
    // TODO:
    // For some reason, this doesn't come through as a mongoose model
    // cc @prashtx
    doc = new Form(doc);
